...
export function App() {
  ...
  const saveQuickBet = async (request: QuickRequest, match: Match) => {
    ...
    const prior = sortBets(bets.filter(item => item.date === activeDate));
    ...
    const bet: Bet = { id: uid(), date: activeDate, matchId: match.id, ruleId: rule.id, selection, note: '', amount: request.amount, sortOrder: insertAt, ruleSnapshot: structuredClone(rule), placedAt: new Date().toISOString(), lateEntry: false, lateReason: '' };
    await save({ ...store, bets: [...bets.filter(item => item.date !== activeDate), ...shifted, bet] });
